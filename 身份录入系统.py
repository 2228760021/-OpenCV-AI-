...
class DataRecordUI(QWidget):  # 记录UI数据
    receiveLogSignal = pyqtSignal(str)

    def __init__(self):
        super(DataRecordUI, self).__init__()  # 用init方法调用为父类的DataRecordUI
        loadUi('./ui/DataRecord.ui', self)   # 读取UI布局
        self.setWindowIcon(QIcon('./icons/icon.png'))  # 设置窗体的图标
        self.setFixedSize(1011, 601)  # 设置窗体的大小
        ...
        # 图像捕获
        self.isExternalCameraUsed = False  # 使用外部摄像头捕获图像
        self.useExternalCameraCheckBox.stateChanged.connect(
            lambda: self.useExternalCamera(self.useExternalCameraCheckBox))   # lambda作为表达式定义匿名函数
        self.startWebcamButton.toggled.connect(self.startWebcam)  # 网络摄像头捕获图像
        self.startWebcamButton.setCheckable(True)

        # 定时器
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.updateFrame)  # 每帧画面根据时间更新

        # 人脸检测
        self.isFaceDetectEnabled = False  # 已启用人脸检测为失败时
        self.enableFaceDetectButton.toggled.connect(self.enableFaceDetect)  # 启用人脸检测按钮→启用人脸检测
        self.enableFaceDetectButton.setCheckable(True)  # 设置为可以检测

        # 数据库
        self.database = './FaceBase.db'  # 数据库地址
        self.datasets = './datasets'    # 数据集地址
        self.isDbReady = False  # 数据库还未就绪
        self.initDbButton.setIcon(QIcon('./icons/warning.png'))  # 初始化数据库按钮所设置图标的地址
        self.initDbButton.clicked.connect(self.initDb)   # 点击初始化数据库按钮→初始数据库

        # 用户信息
        self.isUserInfoReady = False   # 用户信息准备还未就绪
        self.userInfo = {'stu_id': '', 'cn_name': '', 'en_name': ''}  # 学号 姓名（中文） 姓名拼音首字母（英文）

        # 点击添加或更新用户信息按钮→添加或更新用户信息
        ...
        self.minFaceRecordCount = 100  # 最少采集100个
    ...
    def initDb(self):
        ...
        try:
            ...
            cursor.execute('''CREATE TABLE IF NOT EXISTS users (
                              stu_id VARCHAR(9) PRIMARY KEY NOT NULL,
...
class UserInfoDialog(QDialog):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon('./ic   ons/icon.png'))
        self.setFixedSize(425, 300)

        # 使用正则表达式限制用户输入
        stu_id_regx = QRegExp('^[0-9]{9}$')
